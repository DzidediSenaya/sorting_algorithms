 ...
 *
 * This function calculates the length of a doubly linked list.
 */
size_t list_length(const deck_node_t *list)
{
    size_t len = 0;
 ...
 *         Zero if card1 == card2
 *
 * This function is used as a comparison function for qsort to compare two cards
 * based on their values and kinds. It returns a positive number if card1 should
 * appear after card2 in the sorted order, a negative number if card1 should
 * appear before card2, and zero if they have equal values and kinds.
 */
int compare_cards(const void *card1, const void *card2)
{
 ...
 *
 * This function maps the card values to their corresponding indices
